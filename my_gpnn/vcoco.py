    ...
    for i, (edge_features, node_features, adj_mat, node_labels, node_roles, boxes, img_ids, img_names, human_nums, obj_nums, classes) in enumerate(val_loader):
        ...
        if args.visualize:
            ...
            for j, image_id in enumerate(img_ids):
                ## TODO your path to MS-COCO val2014 images
              
                
                image_path = os.path.join(args.image_root, f"COCO_val2014_{image_id:012d}.jpg")
                ...
                for n, (batch, human_indice, obj_indice) in enumerate(best_det_indices):
                    ...
                    if pred_action != None:
                        ...
                        font = ImageFont.truetype('./fonts/FreeSans.ttf', 24)
...
def parse_arguments():
    ...
    ## TODO: 
    # For visulization, 
    # please change the image-root here to your HICO-DET test images
    # and set visualize=True
    parser = argparse.ArgumentParser(description='VCOCO dataset')
    parser.add_argument('--image-root', default='/data1/tangjq/COCO_data/val2014', help='path to your MSCOCO val2014 images')
    parser.add_argument('--project-root', default=paths.project_root, help='test images path')
